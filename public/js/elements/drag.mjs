...
function dragStart(d) {
  ...
  if (
    ...
    if (d3.select(evt.sourceEvent.srcElement).classed("sticky-area")) {
      sel.classed("dragging", true).classed("unmoved", false);
...
let lastBroadcastTime = 0; // Track the last broadcast time
...
function dragging(d) {
  ...
  if (!sel.classed("matrix")) {
    const hits = [];
    d3.selectAll(
      "div.group:not(.dragging), div.note:not(.dragging), div.card:not(.dragging)"
    )
      .filter(function () {
        return this !== node;
      })
...
async function dragEnd(d) {
  const sel = d3.select(this).classed("dragging", false);
  ...
  if (hit.node()) {
    ...
    if (hit.classed("group")) {
      ...
      if (Array.isArray(pipe_to) && pipe_to?.length)
        pipes = [...pipes, ...pipe_to];
    ...
    // THE OBJECT IS MOVED OUT OF ALL GROUPS
