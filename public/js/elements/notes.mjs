...
const nColors = 8;
const colors = d3.scaleOrdinal(d3.schemePastel1).domain(d3.range(nColors));
...
export const Note = {
  add: async function (_kwargs) {
    ...
    if (!id)
      datum = await POST("/addNote", {
        data: { content, color, x, y, tree: ntree, pipe_from },
        project: wallId,
      });
    ...
    if (!parent) {
      if (child) {
        const parentNode = d3
          ...
          .filter(
            (d) =>
              d.tree === tree.moveUp(ntree) && d.id === +tree.getLeaf(ntree)
          )
          .node();
    ...
    const note = parent
      ...
      .styles({
        transform: (d) =>
          ![null, undefined].includes(d.x) && ![null, undefined].includes(d.y)
    ...
    note
      .addElems("div", "color-swatches")
      .addElems(
        "div",
        "color",
        d3.range(nColors).map((d) => colors(d))
      )
  ...
  update: async function (_kwargs) {
    ...
    if (!note) {
      ...
      if (id) {
        note = d3.selectAll("div.note").filter((d) => d.id === id);
      } else if (pipe_from) {
        note = d3
          .selectAll("div.note")
    ...
    if (!child && !note.node().parentNode.classList.contains("canvas")) {
      ...
      const expectedParent = d3
        .selectAll(".group")
        .filter(
          (d) => d.tree === tree.moveUp(ntree) && d.id === +tree.getLeaf(ntree)
        );
      if (
        expectedParent.node() &&
        note.node().parentNode !== expectedParent.node()
      ) {
    ...
    note.select("textarea").each(function (d) {
      this.value = d.content;
    });
      ...
      if (group_pipes?.length) {
        ...
        for (let p = 0; p < group_pipes.length; p++) {
          ...
          const {
            ...
            x: forget_x,
            y: forget_y,
            pipe_from: forget_piping,
            ...data_to_pass
          } = note.datum();
          await constructorRef.update({
            datum: {
              ...data_to_pass,
              ...{ tree: ptree, x: null, y: null, pipe_from: nid },
            },
            bcast,
          });
        }
  ...
  remove: async function (_kwargs) {
    ...
    if (bcast) {
      ...
      constructorRef.broadcast({
        operation: "delete",
        data: { id: note.datum().id },
      });
    }
      ...
      while (i >= 0) {
        ...
        if (group.node()) {
          ...
          const children = group
            .selectAll("div.group, div.note, div.card")
            .size();
          if (children <= 1) {
            await Group.remove({
              group,
  ...
  releaseAll: function (bcast) {
    ...
    d3.selectAll("div.note.focus").each(function (d) {
      const { id } = d;
